...
import { UserWithPreferenceAndStaffings } from "../schedule-generator-types/user";
import { SplitDate } from "../../../shared/types/splitDate";
import { getAvailabilityforDate } from "./helper-functions";
import { getBackupForDate, getStaffingsOnStaffRequired, getUserStaffings, getUserStaffingsForWeek } from "./database-actions";
...
export const checkEnoughBackupStaff = async (date: Date) => {
  return (await getBackupForDate(date)).length >= 2;
}
