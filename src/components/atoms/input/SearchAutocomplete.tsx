...
export function SearchAutocomplete<T extends object>(props: ComboBoxProps<T>) {
  ...
  return (
    <div className="inline-flex flex-col relative mt-4 w-52">
      ...
      <div
        ref={outerRef}
        className={`relative px-2 flex inline-flex flex-row items-center rounded-md overflow-hidden shadow-sm border-2 ${state.isFocused ? "border-pink-500" : "border-gray-300"
          }`}
      >
        <input
          {...inputProps}
